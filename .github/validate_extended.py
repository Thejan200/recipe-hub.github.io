...
uk_ids = {
    ...
    "lamb-curry", "sausage-casserole", "bubble-and-squeak", "welsh-rarebit", "scotch-eggs",
    "full-english-breakfast", "kedgeree", "fish-and-chips", "coronation-chicken", "ploughmans-lunch",
    "bread-pudding", "sticky-toffee-pudding"
}
assert uk_ids.issubset(runtime_ids), "British classics batches must be runtime-published"
assert len(uk_ids) == 35, "British classics collection must contain the 35 approved recipes"
for rid in uk_ids:
    recipe = by_id[rid]
...
assert "newest.slice(0,3)" in app and "b.index-a.index" in app, "Home latest recipes must be date-sorted with newest catalog entries winning date ties"
